...
export class ArrayCreator {
  ...
  public static setGeometry() {
    let positions = new Float32Array(ArrayCreator.geometryCreator());
    let matrix = this.operator.xRotation3D(Math.PI);
    matrix = this.operator.translate(matrix, -50, -75, -15);

    for (var ii = 0; ii < positions.length; ii += 3) {
      var vector = this.operator.transformPoint(matrix, [
        positions[ii + 0],
        positions[ii + 1],
        positions[ii + 2],
        1,
  ...
  public static geometryCreator(): number[] {
    return [
      // left column front
